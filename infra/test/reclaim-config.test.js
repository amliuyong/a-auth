...
test('c10_5_persistent_identity_tables_are_durable_without_ttl', () => {
  const app = new App();
  const assetPath = path.resolve(__dirname);
  const stack = new AgentAuthStack(app, 'C105PersistentIdentityTest', {
  ...
  for (const prefix of [
    ...
    assert.equal(
      ...
      undefined,
      `${prefix} must not expire persistent identities through TTL`,
    );
  }
});
